...
lodash(third_party_assets).forEach((assets, type) => {
	if (type === "css_js") {
		lodash(assets).forEach((plugin) => {
			const [name, assetList, css, js] = [
				plugin["name"],
				plugin["assets"],
				[],
				[]
			...
			lodash(assetList).forEach((asset) => {
				const ass = asset.split(",");

				for (let index = 0; index < ass.length; ++index) {
					if (ass[index].slice(ass[index].length - 3) === ".js") {
						js.push(ass[index]);
					} else {
						css.push(ass[index]);
					}
...
// copy all fonts
const fontsOutDirectory = `${ folder.dist_assets }fonts`;
mix.copyDirectory(`${ folder.src }fonts`, fontsOutDirectory);

// copy all images
const imagesOutDirectory = `${ folder.dist_assets }images`;
mix.copyDirectory(`${ folder.src }images`, imagesOutDirectory);
...
// Invoice
mix.sass("resources/scss/custom/pages/_invoice.scss", `${ folder.dist_assets }invoice/invoice.css`);
...
const jsOutDirectory = `${ folder.dist_assets }js/`;
lodash(app_pages_assets).forEach((assets) => {
	for (let index = 0; index < assets.length; ++index) {
		mix.js(assets[index], `${ jsOutDirectory }pages`);
	}
