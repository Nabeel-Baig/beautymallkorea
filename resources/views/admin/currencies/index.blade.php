...
@section('content')
			...
			<div class="card">
				<div class="card-body">
					<div class="float-right">
						@can(PermissionEnum::CURRENCY_DELETE->value)
							<button type="button" class="btn btn-danger" id="delete_all">Delete Selected</button>
						@endcan

						@can(PermissionEnum::CURRENCY_CREATE->value)
							<a class="btn btn-info" href="{{ route('admin.currencies.create') }}">Add</a>
						@endcan
					</div>
	...
	@can(PermissionEnum::CURRENCY_SHOW->value)
...
@section('script-bottom')
	<script>
		$(function () {
			const source = `{{ route("admin.currencies.paginate") }}`;
			...
			$(document, this).on("click", ".view", function () {
				let id = $(this).attr("id");
				let url = "{{ route('admin.currencies.view', ':id') }}";
				$.ajax({
			...
			$(document).on("click", okDeleteSelector, function () {
				let url = '{{ route('admin.currencies.delete', ':id') }}';
				$.ajax({
					type: "delete",
			...
			document.getElementById("delete_all").addEventListener("click", () => {
				...
				if (checkboxes.length > 0) {
					...
					fetch(`{{ route('admin.currencies.delete.many') }}`, {
						method: "delete",
						headers: {
