...
@section('script-bottom')
	<script>
		$(function () {
			const source = `{{ route("admin.quickcategories.paginate") }}`;
			...
			$(document).on("click", okDeleteSelector, function () {
				const url = '{{ route('admin.quickcategories.destroy', ':id') }}';
				$.ajax({
					type: "delete",
			...
			document.getElementById("delete_all").addEventListener("click", () => {
				...
				if (checkboxes.length > 0) {
					...
					fetch(`{{ route('admin.quickcategories.massDestroy') }}`, {
						method: "delete",
						headers: {
