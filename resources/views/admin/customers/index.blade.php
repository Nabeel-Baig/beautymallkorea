...
@section('content')
			...
			<div class="card">
				<div class="card-body">
					<div class="float-right">
						@can(PermissionEnum::CUSTOMER_DELETE->value)
							<button type="button" class="btn btn-danger" id="delete_all">Delete Selected</button>
						@endcan

						@can(PermissionEnum::CUSTOMER_CREATE->value)
							<a class="btn btn-info" href="{{ route('admin.customers.create') }}">Add</a>
						@endcan
					</div>
	...
	@can(PermissionEnum::CUSTOMER_SHOW->value)
...
@section('script-bottom')
	<script>
		$(function () {
			const source = `{{ route("admin.customers.paginate") }}`;
			...
			document.getElementById("delete_all").addEventListener("click", () => {
				...
				if (checkboxes.length > 0) {
					...
					fetch(`{{ route('admin.customers.massDestroy') }}`, {
						method: "delete",
						headers: {
