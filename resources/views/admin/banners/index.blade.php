				...
				<div class="card-body">
					<div class="float-right">
						...
						@can(PermissionEnum::OPTION_CREATE->value)
							<a class="btn btn-info" href="{{ route('admin.banners.create') }}">Add</a>
						@endcan
					</div>
...
@section('script-bottom')
	<script>
		$(function () {
			const source = `{{ route("admin.banners.paginate") }}`;
			...
			$(document).on("click", okDeleteSelector, function () {
				const url = '{{ route('admin.banners.destroy', ':id') }}';
				$.ajax({
					type: "delete",
			...
			document.getElementById("delete_all").addEventListener("click", () => {
				...
				if (checkboxes.length > 0) {
					...
					fetch(`{{ route('admin.banners.massDestroy') }}`, {
						method: "delete",
						headers: {
