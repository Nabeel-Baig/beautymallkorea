							...
							<div class="tab-pane active" id="general" role="tabpanel">
								<div class="mb-3">
									...
									<input type="text" name="product[name]" id="product_name"
										   value="{{ $model->name ?? old("product.name") }}" minlength="3"
										   maxlength="50"
										   class="form-control @error('product.name') parsley-error @enderror" required>
									@error('product.name')
								...
								<div class="mb-3">
									...
									<textarea name="product[description]" id="product_description" minlength="3"
											  class="form-control @error('product.description') parsley-error @enderror"
											  required>{{ $model->description ?? old("product.description") }}</textarea>
								...
								<div class="mb-3">
									...
									<input type="text" name="product[meta_title]" id="product_meta_title"
										   value="{{ $model->meta_title ?? old("product.meta_title") }}" minlength="3"
										   maxlength="50"
										   class="form-control @error('product.meta_title') parsley-error @enderror">
								...
								<div class="mb-3">
									...
									<input type="text" name="product[meta_description]" id="product_meta_description"
										   ...
										   class="form-control @error('product.meta_description') parsley-error @enderror">
								...
								<div class="mb-3">
									...
									<input type="text" name="product[meta_keywords]" id="product_meta_keywords"
										   value="{{ $model->meta_keywords ?? old("product.meta_keywords") }}"
										   minlength="3" maxlength="50"
										   class="form-control @error('product.meta_keywords') parsley-error @enderror">
								...
								<div class="mb-3">
									...
									<select name="tags[]" id="product_tags" style="width: 100%"
											class="form-control select2 @error('tags') parsley-error @enderror"
											multiple>
										@foreach($tags as $tag)
											<option value="{{ $tag->id }}"
													@if(in_array($tag->id, $assignedTags, true)) selected @endif>{{ $tag->name }}</option>
										@endforeach
									</select>
									@error("tags")
									<span class="text-red">{{ $message }}</span>
							...
							<div class="tab-pane" id="links" role="tabpanel">
								<div class="mb-3">
									...
									<select name="categories[]" id="product_categories" style="width: 100%"
											class="form-control select2 @error('categories') parsley-error @enderror"
											multiple>
										@foreach($categories as $category)
											<option value="{{ $category->id }}"
													@if(in_array($category->id, $assignedCategories, true)) selected @endif>{{ $category->name }}</option>
										@endforeach
									</select>
									@error("categories")
									<span class="text-red">{{ $message }}</span>
								...
								<div class="mb-3">
									...
									<select name="related_products[]" id="product_related_products" style="width: 100%"
											class="form-control select2 @error('related_products') parsley-error @enderror"
											multiple>
										@foreach($relatedProducts as $relatedProduct)
											<option value="{{ $relatedProduct->id }}"
													@if(in_array($relatedProduct->id, $assignedRelatedProducts, true)) selected @endif>{{ $relatedProduct->name }}</option>
										@endforeach
									</select>
							...
							<div class="tab-pane" id="options" role="tabpanel">
								<div class="mb-3">
									<div class="row">
										...
										<div class="col-2">
											<button type="button"
													class="btn btn-success waves-effect waves-light mr-1 w-100"
													onclick="productOptionsManagement.addProductOption()">Add Product
												Option
											</button>
										</div>
									</div>
								...
								<div id="product-available-option">
									...
									@foreach($assignedProductOptionValues as $optionId => $productOptionValueGroup)
										<div id="product-available-option-{{ $optionId }}"
											 class="card border border-light border-3">
											<div class="card-body">
												<div class="row mb-3">
													<div class="col-10">
														<label class="form-label">Option Name</label>
														<input data-option-name="{{ $optionId }}" type="text"
															   class="form-control" disabled>
													</div>
													<div class="col-2 d-flex align-items-end">
														<div class="w-100">
															<button title="Delete" type="button"
																	class="w-100 btn btn-danger inner"
																	onclick="productOptionsManagement.removeProductOption({{ $optionId }})">
												...
												<div id="product-option-{{ $optionId }}-option-value">
													@foreach($productOptionValueGroup as $productOptionValue)
														<div
															id="product-option-value-{{ $productOptionValueIndexCounter }}"
															class="row mb-3">
															<div class="col-3">
																...
																<select
																	name="options[{{ $productOptionValueIndexCounter }}][option_value_id]"
																	class="form-control">
																	@foreach($possibleOptionValuesForThisOption as $possibleOptionValue)
																		<option value="{{ $possibleOptionValue->id }}"
																				@if($possibleOptionValue->id === $productOptionValue->id) selected @endif>{{ $possibleOptionValue->name }}</option>
																	@endforeach
																</select>
															</div>
															<div class="col-2">
																...
																<input type="number"
																	   name="options[{{ $productOptionValueIndexCounter }}][quantity]"
																	   value="{{ $productOptionValue->pivot->quantity }}"
															...
															<div class="col-2">
																...
																<select
																	...
																	<option
																		...
																		@if ($productOptionValue->pivot->price_adjustment === ProductOptionPriceAdjustment::POSITIVE->value) selected @endif>{{ ProductOptionPriceAdjustment::POSITIVE->name }}</option>
																	<option
																		value="{{ ProductOptionPriceAdjustment::NEGATIVE->value }}"
																		@if ($productOptionValue->pivot->price_adjustment === ProductOptionPriceAdjustment::NEGATIVE->value) selected @endif>{{ ProductOptionPriceAdjustment::NEGATIVE->name }}</option>
																</select>
															</div>
															<div class="col-2">
															...
															<div class="col-1 d-flex align-items-end">
																<div class="w-100">
																	<button title="Delete" type="button"
																			class="w-100 btn btn-danger inner"
																			onclick="productOptionsManagement.removeProductOptionValue({{ $productOptionValueIndexCounter }})">
												...
												<div id="product-option-add-{{ $optionId }}">
													<button type="button"
															...
															onclick="productOptionsManagement.addProductOptionValue({{ $optionId }})">
